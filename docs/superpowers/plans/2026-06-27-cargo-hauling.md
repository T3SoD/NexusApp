...
- **Branch:** all work on `feature/cargo-hauling` (already created off `main @ 61c837d`). Never commit/push/merge/release without the owner's explicit per-action "yes". A "yes" is per-commit and does not carry forward.
...
- **WPF cannot be built in WSL.** The testable core (Tasks 1-4) is verified with `dotnet test`. UI tasks (5-8) compile-verify via CI build-check on push and runtime-verify by the owner on Windows (`~/test_nexus.ps1`).
...
Run: `cd /home/owner/NexusApp && dotnet build NexusApp.Tests/NexusApp.Tests.csproj`
Expected: build succeeds (the test project references the main project, so this compiles `Haul.cs`).

- [ ] **Step 3: Commit** (ask the owner first per the git gate)
...
Run: `cd /home/owner/NexusApp && dotnet test NexusApp.Tests/NexusApp.Tests.csproj --filter HaulLogParserTests`
...
Run: `cd /home/owner/NexusApp && dotnet test NexusApp.Tests/NexusApp.Tests.csproj --filter HaulTrackerTests`
...
Run: `cd /home/owner/NexusApp && dotnet test NexusApp.Tests/NexusApp.Tests.csproj`
...
Run: `cd /home/owner/NexusApp && dotnet test NexusApp.Tests/NexusApp.Tests.csproj`
Expected: PASS. (Full WPF app build happens in CI / on the owner's Windows box; it cannot build in WSL.)
...
- [ ] **Step 8: Commit** (ask the owner first), then the owner build-verifies on Windows.
...
- [ ] **Step 8: Commit** (ask the owner first), then the owner runtime-verifies the overlay on Windows.
...
## Final verification (before any merge/release - both need explicit yes from the owner)

- [ ] `cd /home/owner/NexusApp && dotnet test NexusApp.Tests/NexusApp.Tests.csproj` - all green.
- [ ] Push branch (ask first) so CI build-check compiles the full WPF app.
- [ ] the owner runtime-verifies on Windows: accept a hauling contract in-game, confirm the main-window Hauling tab and the overlay HAULING tab populate, legs flip to done, the consolidation groupings read correctly, a completed haul moves to Finished, and `[HAUL]` lines show in the App Log Monitor.
