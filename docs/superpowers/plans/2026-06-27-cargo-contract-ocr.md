...
- [ ] **Step 3:** In `OnStartup` after the `Shards` block: construct `ContractOcr = new ContractOcrService();` apply the saved region if present (`if (Settings.Current.ContractRegion is {} r) ContractOcr.SetRegion(r.X, r.Y, r.Width, r.Height);`); `ContractScan = new ContractScanner(ContractOcr);` wire `ContractScan.ContractScanned += d => Hauls.ApplyContractDetails(d);` and `if (Settings.Current.AutoScanContracts) ContractScan.Start();`.
