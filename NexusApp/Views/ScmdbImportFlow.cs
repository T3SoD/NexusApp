...
/// name resolution the Game.log importer uses (<see cref="GameLogSession.ResolveName"/>), marks
/// the newly recognized ones owned, and shows a summary. FILE IMPORT ONLY - no network surface of
/// any kind. ADD-ONLY: never un-marks a blueprint. Mirrors <see cref="BlueprintImportFlow"/>'s
/// shape (file -> parse -> resolve -> apply -> refresh -> summary) as a small, separate flow so
/// the Game.log import stays untouched.
/// </summary>
public static class ScmdbImportFlow
{
    ...
    /// no background thread or busy state is needed). Cancelling the file picker logs nothing and
    /// shows nothing further.</summary>
    public static void Run(Window owner)
    {
        ...
        new ScmdbImportResultDialog(imported, plan.AlreadyOwned.Count, plan.Unrecognized,
